...
// 返回最大的总价值
void func(int time_left, int total_val, int cur_idx) {
    // 没得选或者背包满了
    ...
    // 不选 choice[cur_idx]
    func(time_left, total_val, cur_idx + 1);
    // 选 choice[cur_idx]，前提不超过背包容量
    if (time_left >= choice[cur_idx][0])
        func(time_left - choice[cur_idx][0], total_val + choice[cur_idx][1], cur_idx + 1);
}
