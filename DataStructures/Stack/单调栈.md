...
**单调栈（Monotonic Stack）**是一种在传统栈的基础上增加了“单调性”的数据结构。它的核心思想是：**栈内的元素只能单调递增或单调递减**。

当你需要在一维数组中，为每个元素寻找左边或右边**第一个比它大（或小）**的元素时，单调栈几乎是唯一的也是最高效的解法。它能将原本需要 $O(N^2)$ 的暴力双重循环，优化到惊人的 **$O(N)$ 线性时间复杂度**。

### 一、核心原理与分类

单调栈分为两种类型。需要注意的是，关于“递增”和“递减”的定义，通常是指**从栈底到栈顶**的元素顺序：

#### 1. 单调递增栈
* **特点**：从栈底到栈顶，元素逐渐变大。
* **操作规则**：新元素准备入栈时，如果它比栈顶元素**小**，就把栈顶元素弹出，直到栈顶元素比它小（或栈空），再将新元素入栈。
* **适用场景**：寻找下一个/上一个更**小**的元素。

#### 2. 单调递减栈
* **特点**：从栈底到栈顶，元素逐渐变小。
* **操作规则**：新元素准备入栈时，如果它比栈顶元素**大**，就把栈顶元素弹出，直到栈顶元素比它大（或栈空），再将新元素入栈。
* **适用场景**：寻找下一个/上一个更**大**的元素。

### 二、为什么能达到 $O(N)$？

在遍历数组时，每个元素都只有两种命运：
1. **入栈**一次。
2. 被后面的元素“逼”出栈，即**出栈**一次。

因为**每个元素最多进栈一次、出栈一次**，所以整个算法的总体时间复杂度是 $O(N)$，尽管内部包含了一个 `while` 循环。

### 三、C++ 代码模板

以经典问题“**寻找右边第一个比当前元素大的数**”（使用单调递减栈）为例，其核心代码模板如下：
...
std::vector<int> nextGreaterElement(const std::vector<int>& nums) {
    int n = nums.size();
    std::vector<int> result(n, -1); // 默认找不到则为 -1
    std::stack<int> st;            // 栈中通常存元素的【索引】，方便定位

    for (int i = 0; i < n; ++i) {
        // 当栈不为空，且当前元素大于栈顶索引对应的元素时
        while (!st.empty() && nums[i] > nums[st.top()]) {
            result[st.top()] = nums[i]; // 当前元素就是栈顶元素右边第一个比它大的数
            st.pop();                   // 弹出栈顶
        }
...
> **记忆口诀**
>
> - 找**大**的，用**递减**栈；找**小**的，用**递增**栈。
> - 从左往右遍历找右侧目标；从右往左遍历（或利用当前出栈状态）找左侧目标。
