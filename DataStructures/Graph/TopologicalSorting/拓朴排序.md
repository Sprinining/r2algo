...
拓扑排序是图论中针对 DAG（有向无环图）的经典算法。其核心任务是将图中的所有节点排成一个线性序列，使得对于图中的任意一条有向边 u -> v，在最终序列中节点 u 必须排在节点 v 的前面。
#### 一、 核心本质：带限制的层序遍历
在结构上，拓扑排序的骨架与**树/图的层序遍历（BFS）**完全一致，都是利用队列实现由点及面的层层推进。
它们之间最决定性的区别在于入队限制：

- **普通层序遍历 (BFS)**：无条件限制。只要邻居节点未被访问过`（visited[v] == false）`，就直接将其推入队列。
- **拓扑排序**：有因果逻辑限制。邻居节点不能直接入队，必须等到它的入度（Indegree）减为 0 时，才允许将其推入队列。入度代表当前节点有多少个“前置条件”。只有当前置条件全部被拔除（入度清零），该节点的大门才会打开，才能进入队列参与下一层的遍历。

- **Dijkstra 算法 (最短路)**：带了优先级限制（权值小的先出队）的层序遍历（用优先队列代替普通队列）。

#### 二、 算法实现流程（广度优先迭代法）
拓扑排序利用静态数组模拟队列，通过“不断解锁”的方式实现层序遍历：

1.	统计入度：遍历整个图，统计每个节点的初始入度。
2.	源点入队：将所有入度为 0 的节点（没有任何前置依赖的起点）扔进队列。
3.	循环减度：
   • 从队头弹出一个节点 u（代表该节点已被处理/解锁）。
   • 遍历从 u 出发的所有边，找到其后继邻居节点 v。
   • 将 v 的入度减 1（代表 v 的其中一个前置条件已被满足）。
   • 【关键限制】：若减 1 后 v 的入度变为 0，则将 v 扔进队列。
4.	结束条件：队列为空时遍历结束。

#### 三、 拓扑排序的两大衍生应用
除了用于常规的先后顺序规划（如选课系统、编译依赖），拓扑排序在算法中还隐藏着两个极其重要的功能：

1. 有向图判环（死锁检测）
  在拓扑排序结束时，可以通过判断最终出队的节点总数是否等于图中原本的总节点数。
  • 如果出队总数 < N，说明图中有环存在。
  • 原理：环内的节点由于互相依赖，入度永远不可能减到 0，因此它们会被无限“死锁”在队列之外。
2. 图论 DP 的天然递推基石
  在有向无环图（DAG）上做动态规划时，最核心的难点在于确定状态转移的顺序。
  • 拓扑排序严格保证了：当你开始处理节点 v 时，所有能够到达 v 的前驱节点 u 必然已经全部出队并处理完毕。
  • 这为自底向上的递推 DP 提供了绝对安全的计算顺序，避免了后效性。

  • 拓扑排序在 DAG 上就是严格位置依赖的、自底向上的 DP。
