...
滑动窗口（Sliding Window）不仅是一种代码结构，更是一种基于**单调性**和**增量更新**的算法模型。其底层核心在于通过空间换时间，将二维空间的穷举压缩到一维线性的扫描。

### 一、 核心本质：空间换时间与单调性映射

#### 1. 核心机制：左右边界不回退 $\rightarrow$ 状态的增量更新
* 左右指针均不回退，本质上是**将 $O(N^2)$ 的全量遍历，转化为了 $O(N)$ 的状态增量更新**。
* 右边界右移是“纳新”，左边界右移是“去旧”。窗口内的状态（如哈希表、当前和）只做 `+1` 或 `-1` 的微调，用常数时间的操作，承接了边界的单调移动。

#### 2. 核心前提：范围与指标之间的“绝对单调性”
* **区域的扩大（或缩小），其对应的答案指标必须具有单调递增或递减的性质**。这是左右边界不回退的数学底气。
* **反例（何时失效）**：如果数组中包含**负数**，求“和等于 $K$ 的子数组”，扩大范围可能让和变小，缩小范围可能让和变大（失去了单调性）。此时滑动窗口失效，必须退化为**前缀和 + 哈希表**。

#### 3. 解题视角：从“寻找一个区间”切换为“以当前位置结尾的合法区间”
* 滑动窗口不是漫无目的地在数组里“乱滑”，它的物理意义是：**当右指针固定在 `right` 时，去寻找满足条件的最左边界 `left`**。
* 因此，滑动窗口本质上也是在穷举“以每个位置结尾情况下的答案”，只是它巧妙地利用了上一次的 `left` 结果作为基础，省去了从头扫描的步骤。

### 二、 两种收缩策略的底层逻辑

根据“范围与指标的单调关系”，代码控制逻辑通常分化为以下两种对偶状态：

#### 1. 求最大区间（尽可能撑大窗口）
* **底层逻辑**：右指针狂奔直到违规，左指针才被迫跟进，直到重新合规。
* **策略**：条件满足时，右指针一直右移以扩大范围；一旦不满足，左指针右移直至刚好处在“合规的临界点”，此时结算最大值。

#### 2. 求最小区间（尽可能压榨窗口）
* **底层逻辑**：右指针只要刚一合规，左指针就迫不及待地向前试探临界点。
* **策略**：右指针右移直到“刚巧满足条件”，立刻定格；然后左指针开始右移，直到“刚好不满足条件”的前一步，此时结算最小值。

### 三、 数学高度抽象

滑动窗口的执行逻辑，可以完美抽象为三个数学句式的循环：

$$
\begin{aligned}
1. & \quad \text{Explore: } \text{right} \to \text{ 引入新状态} \\
2. & \quad \text{Restrict: } \text{while (状态违反单调性边界) } \to \text{left} \to \text{ 移出旧状态} \\
3. & \quad \text{Record: } \text{结算以当前 right 为右端点的最值指标}
\end{aligned}
$$

抓住“**不回退**”与“**单调性**”这两根铁轨，就能看穿所有子数组、子串相关问题的滑动窗口本质。
